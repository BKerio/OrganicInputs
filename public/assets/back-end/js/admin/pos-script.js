...
function renderCustomerAmountForPay() {
    ...
    if (national.length < 9) {
        wrapper.attr("title", "Enter M-Pesa phone number").attr("data-bs-toggle", "tooltip");
...
let posMpesaCurrentCheckoutId = null;
...
const POS_MPESA_STATUS_UI = {
    ...
    pending: { icon: "", color: "", titleKey: "pending", showRetry: false },
};
...
function resetPosMpesaModal() {
    ...
    $("#pos-mpesa-status-message").text(
        $("#route-admin-pos-mpesa-stk-push").data("pending-text") || labels.pending
    );
...
function showPosMpesaPendingModal() {
    resetPosMpesaModal();
    const modalEl = document.getElementById("posMpesaPendingModal");
    if (modalEl) {
...
function completePosOrder(checkoutRequestId) {
    ...
    $.post({
        url: $("#route-admin-pos-mpesa-complete-order").data("url"),
        data: {
        ...
        success: function (response) {
            if (Boolean(response.checkProductTypeForWalkingCustomer) === true) {
                ...
                const addCustomerOffcanvas = document.getElementById(
                    "offcanvasAddNewCustomer"
                );
                bootstrap.Offcanvas.getOrCreateInstance(addCustomerOffcanvas).show();
...
function pollPosMpesaStatusOnce(checkoutRequestId, startedAt) {
    const statusUrl = $("#route-admin-pos-mpesa-status").data("url");
    const labels = getPosMpesaLabels();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.message) {
                $("#pos-mpesa-status-message").text(data.message);
            }
        },
        error: function (xhr) {
            console.error("POS M-Pesa status poll error", xhr.status, xhr.responseText);
        },
    });
}

function pollPosMpesaStatus(checkoutRequestId) {
    posMpesaCurrentCheckoutId = checkoutRequestId;
...
function startPosMpesaPayment() {
    ...
    $.post({
        url: $("#route-admin-pos-mpesa-stk-push").data("url"),
        data: formData,
        contentType: false,
        processData: false,
        beforeSend: function () {
