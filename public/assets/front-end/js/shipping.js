...
function checkoutFromShipping() {
    ...
    $.post({
        ...
        error: function (data) {
            let errorMsg = data.responseJSON?.errors;
            if (typeof errorMsg === 'string') {
                toastr.error(errorMsg, { CloseButton: true, ProgressBar: true });
            } else if (Array.isArray(errorMsg)) {
                errorMsg.forEach(function (item) {
                    toastr.error(item.message || item, { CloseButton: true, ProgressBar: true });
                });
            } else if (errorMsg && typeof errorMsg === 'object') {
                toastr.error(JSON.stringify(errorMsg), { CloseButton: true, ProgressBar: true });
            } else {
                toastr.error('Could not save delivery details. Please try again.', {
                    CloseButton: true,
                    ProgressBar: true
                });
            }
