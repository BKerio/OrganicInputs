...
use App\Http\Controllers\Payment_Methods\MpesaPaymentController;
...
if (!$isGatewayPublished) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'mpesa', 'as' => 'mpesa.'], function () {
            Route::match(['get', 'post'], 'pay', [MpesaPaymentController::class, 'pay'])->name('pay');
            Route::get('status', [MpesaPaymentController::class, 'status'])->name('status');
            Route::get('complete', [MpesaPaymentController::class, 'complete'])->name('complete');
            Route::post('callback', [MpesaPaymentController::class, 'callback'])->name('callback')
                ->withoutMiddleware([VerifyCsrfToken::class]);
        });
