...
class OrderManager
{
        ...
        if ($order['payment_method'] == 'cash_on_delivery' || $order['payment_method'] == 'offline_payment') {
            ...
            if ($order['seller_is'] == 'admin') {
                ...
                if ($shipping_model == 'sellerwise_shipping') {
                    ...
                    $wallet->collected_cash += ($currentOrderAmount - $commission);
                } else {
    ...
    public static function getTotalCouponAmount($request, $couponCode): array
    {
        ...
        $coupon = null;
        if ($firstCoupon['coupon_type'] == 'first_order') {
            $coupon = $firstCoupon;
        } elseif ($firstCoupon['limit'] > $couponLimit) {
            $coupon = $firstCoupon;
        } else {
            return [
                'status' => false,
    ...
    private static function buildOrderConfirmationSmsContext(
        ...
        $address = $addressId ? ShippingAddress::find($addressId) : null;
        $customer = $customerInfo['customer'] ?? null;
        $registeredCustomer = ($customer !== 'offline' && is_object($customer)) ? $customer : null;

        if (!$registeredCustomer && !empty($customerInfo['newCustomerRegister'])) {
            $registeredCustomer = is_object($customerInfo['newCustomerRegister'])
                ? $customerInfo['newCustomerRegister']
                : User::find($customerInfo['newCustomerRegister']['id'] ?? $customerInfo['new_customer_id'] ?? null);
        }

        $fullName = trim($address?->contact_person_name ?? '');
        if ($fullName === '' && $registeredCustomer) {
            $fullName = trim(($registeredCustomer->f_name ?? '') . ' ' . ($registeredCustomer->l_name ?? ''));
        }
        if ($fullName === '') {
            $fullName = translate('customer');
        }

        $firstName = $registeredCustomer?->f_name
            ?: (str_contains($fullName, ' ') ? explode(' ', $fullName, 2)[0] : $fullName);

        $phone = session('checkout_mpesa_phone')
            ?: ($address?->phone ?? '')
        ...
        $orders = Order::whereIn('id', $orderIds)->get(['id', 'order_amount', 'payment_method']);
        $orderTotal = $orders->sum('order_amount');
        $orderLabel = count($orderIds) === 1
            ? '#' . $orderIds[0]
            : '#' . implode(', #', $orderIds);

        $resolvedPaymentMethod = $paymentMethod ?: ($orders->first()?->payment_method ?? '');
        $paymentLabel = $resolvedPaymentMethod
            ? translate($resolvedPaymentMethod)
            : translate('payment');
        ...
        return [
            ...
            'order_id' => (string) $orderIds[0],
            'order_ids' => $orderLabel,
            'order_total' => webCurrencyConverter(amount: $orderTotal),
        ...
        foreach ($orderProducts as $key => $orderProduct) {
            ...
            if ($product) {
                ...
                if (($product['product_type'] == 'physical') && (($product['current_stock'] < $orderProduct['qty']) || ($product['minimum_order_qty'] > $product['current_stock']))) {
                    $productValid = false;
                    $errorMessages[] = ($product['name'] ?? '') . translate(' cannot be ordered because the available stock is insufficient or does not meet the minimum order quantity requirement.');
                }
                if ($productValid) {
                    $color = null;
                    $choices = [];
                    if ($orderProduct->variation) {
                        $variation = is_array($orderProduct?->variation) ? $orderProduct?->variation : json_decode($orderProduct?->variation ?? '', true);
                        ...
                        for ($i = 0; $i < $count; $i++) {
                            if (json_decode($product->variation)[$i]->type == $orderProduct->variant) {
                                ...
                                if (json_decode($product->variation)[$i]->qty < $orderProduct->qty) {
                                    $productValid = false;
                                    $errorMessages[] = $product['name'] . translate(' variation does not have enough stock to fulfill the requested quantity.');
                                }
                            }
                        }
                    } else {
