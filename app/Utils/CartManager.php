...
class CartManager
{
    ...
    {
        self::assignDefaultOrderWiseShippingIfMissing($groupId, $type);
    ...
    public static function assignDefaultOrderWiseShippingIfMissing($groupId = null, $type = null): void
    {
        self::syncOrderWiseShippingCostsFromMethods($groupId, $type);

        $cartGroupIds = $groupId != null
            ? [$groupId]
            : ($type == 'checked' ? self::get_cart_group_ids(type: 'checked') : self::get_cart_group_ids());
        ...
        foreach ($cartGroupIds as $cartGroupId) {
            ...
            if (self::resolveShippingTypeForCartItem($cartItem, $shippingMethodConfig, $adminShipping) != 'order_wise') {
                continue;
            }

            $methods = self::getOrderWiseShippingMethodsForCartItem($cartItem, $shippingMethodConfig);
            if ($methods->isEmpty()) {
                continue;
            }

            $existing = CartShipping::where('cart_group_id', $cartGroupId)->first();
            $activeMethod = $existing
                ? $methods->firstWhere('id', $existing->shipping_method_id)
                : null;
            $activeMethod = $activeMethod ?? $methods->first();

            if (!$existing) {
                CartShipping::create([
                    'cart_group_id' => $cartGroupId,
                    'shipping_method_id' => $activeMethod->id,
                    'shipping_cost' => $activeMethod->cost,
                ]);
            } elseif (!$methods->firstWhere('id', $existing->shipping_method_id)) {
                $existing->shipping_method_id = $activeMethod->id;
                $existing->shipping_cost = $activeMethod->cost;
                $existing->save();
            }
        }
    }

    public static function syncOrderWiseShippingCostsFromMethods($groupId = null, $type = null): void
    {
        $cartShippings = CartShipping::whereHas('cart', function ($query) use ($type) {
            return $query->where('product_type', 'physical')->whereHas('product', function ($query) {
                return $query->active();
            })->when($type == 'checked', function ($query) {
                return $query->where('is_checked', 1);
            ...
            ->when($groupId == null && $type == 'checked', function ($query) {
                return $query->whereIn('cart_group_id', self::get_cart_group_ids(type: 'checked'));
            })
            ->when($groupId == null && $type != 'checked', function ($query) {
                return $query->whereIn('cart_group_id', self::get_cart_group_ids());
            })
            ->get();

        foreach ($cartShippings as $cartShipping) {
            $method = ShippingMethod::where(['id' => $cartShipping->shipping_method_id, 'status' => 1])->first();
            if ($method && (float) $method->cost !== (float) $cartShipping->shipping_cost) {
    ...
    public static function getCheckoutShippingOptions(?string $type = 'checked'): array
    {
        self::assignDefaultOrderWiseShippingIfMissing(null, $type);

        $shippingMethodConfig = getWebConfig(name: 'shipping_method');
        $adminShipping = ShippingType::where('seller_id', 0)->first();
        $cartItems = self::getCartListQuery(type: $type);
        $items = [];

        foreach ($cartItems->groupBy('cart_group_id') as $groupId => $groupItems) {
            $first = $groupItems->first();
            if (!$first || $first->product_type != 'physical') {
                continue;
            }

            if (self::resolveShippingTypeForCartItem($first, $shippingMethodConfig, $adminShipping) != 'order_wise') {
                continue;
            }

            $methods = self::getOrderWiseShippingMethodsForCartItem($first, $shippingMethodConfig);
            if ($methods->isEmpty()) {
                continue;
            }

            $chosen = CartShipping::where('cart_group_id', $groupId)->first();

            $items[] = [
                'cart_group_id' => $groupId,
                'seller_label' => $first->seller_is == 'admin'
                    ? translate('inhouse_products')
                    : ($first->seller?->shop?->name ?? translate('shop')),
                'methods' => $methods,
                'selected_id' => $chosen?->shipping_method_id ?? $methods->first()->id,
            ];
        }

        if ($shippingMethodConfig == 'inhouse_shipping' && count($items) > 0) {
            $firstItem = $items[0];
            return [
                'is_inhouse' => true,
                'items' => [[
                    'cart_group_id' => 'all_cart_group',
                    'seller_label' => translate('shipping_method'),
                    'methods' => $firstItem['methods'],
                    'selected_id' => $firstItem['selected_id'],
                ]],
            ];
        }
    ...
    private static function resolveShippingTypeForCartItem($cartItem, ?string $shippingMethodConfig = null, $adminShipping = null): string
    {
        $shippingMethodConfig = $shippingMethodConfig ?? getWebConfig(name: 'shipping_method');
        $adminShipping = $adminShipping ?? ShippingType::where('seller_id', 0)->first();

        if ($shippingMethodConfig == 'inhouse_shipping') {
            return $adminShipping->shipping_type ?? 'order_wise';
        }
        ...
        return $sellerShipping->shipping_type ?? 'order_wise';
    }

    private static function getOrderWiseShippingMethodsForCartItem($cartItem, ?string $shippingMethodConfig = null): Collection
    {
        $shippingMethodConfig = $shippingMethodConfig ?? getWebConfig(name: 'shipping_method');

        if ($shippingMethodConfig == 'inhouse_shipping' || $cartItem->seller_is == 'admin') {
            return Helpers::getShippingMethods(1, 'admin');
        }

        return Helpers::getShippingMethods($cartItem->seller_id, $cartItem->seller_is);
    }
    ...
    public static function getAppliedTaxAmount($product = null, $taxConfig = [], $totalDiscountedPrice = 0, $appliedDiscountedAmount = 0): float|int
    {
        ...
        if ($taxType == 'product_wise') {
            if (isset($product['taxVats']) && count($product['taxVats']) > 0) {
            ...
            if (isset($product['category']['taxVats']) && count($product['category']['taxVats']) > 0) {
