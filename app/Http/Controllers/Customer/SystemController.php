...
class SystemController extends Controller
{
    ...
    public function getChooseShippingAddressOther(Request $request): JsonResponse
    {
        ...
        // Shipping start
        $addressId = (int) ($shipping['shipping_method_id'] ?? 0);
        $hasShippingPayload = !empty($shipping['contact_person_name']) && !empty($shipping['phone']);

        if ($hasShippingPayload || isset($shipping['shipping_method_id'])) {
            if ($shipping['contact_person_name'] == null || $shipping['contact_person_name'] === '' || !isset($shipping['address_type']) || empty($shipping['address']) || empty($shipping['city']) || empty($shipping['zip']) || empty($shipping['country']) || empty($shipping['phone']) || ($isGuestCustomer && empty($shipping['email']))) {
                return response()->json([
                    'errors' => translate('Fill_all_required_fields_of_shipping_address')
                ], 403);
            } elseif ($countryRestrictStatus && !self::delivery_country_exist_check($shipping['country']) && !isSimpleKenyaCheckout()) {
                return response()->json([
                    'errors' => translate('Delivery_unavailable_in_this_country.')
                ], 403);
            } elseif ($zipRestrictStatus && !self::delivery_zipcode_exist_check($shipping['zip']) && !isSimpleKenyaCheckout()) {
                return response()->json([
                    'errors' => translate('Delivery_unavailable_in_this_zip_code_area')
                ], 403);
            }
        ...
        } elseif ($hasShippingPayload && $addressId === 0 && !isset($shipping['update_address']) && !isset($shipping['save_address'])) {
            $addressId = ShippingAddress::insertGetId([
                'customer_id' => auth('customer')->id() ?? ((session()->has('guest_id') ? session('guest_id') : 0)),
                'is_guest' => auth('customer')->check() ? 0 : (session()->has('guest_id') ? 1 : 0),
                'contact_person_name' => $shipping['contact_person_name'],
        ...
        if (isset($shipping['phone'])) {
            session()->put('checkout_mpesa_phone', $shipping['phone']);
        }
