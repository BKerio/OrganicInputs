...
class WebController extends Controller
{
    ...
    public function checkout_details(Request $request)
    {
        ...
        CartManager::assignDefaultOrderWiseShippingIfMissing(null, 'checked');
        ...
        return view(VIEW_FILE_NAMES['order_shipping'], [
            ...
            'simpleKenyaCheckout' => isSimpleKenyaCheckout(),
            'checkoutShippingOptions' => CartManager::getCheckoutShippingOptions('checked'),
        ]);
    }

    public function checkout_payment(Request $request): View|RedirectResponse
    {
        if (empty(session('address_id')) && empty(session('billing_address_id'))) {
        ...
        if (!empty(session('address_id')) && !empty(session('billing_address_id'))) {
            $paymentGatewaysList = payment_gateways();
            if (isSimpleKenyaCheckout()) {
                $paymentGatewaysList = $paymentGatewaysList->sortBy(function ($gateway) {
                    return $gateway->key_name === 'mpesa' ? 0 : 1;
                })->values();
            }
            ...
            return view(VIEW_FILE_NAMES['payment_details'], [
                ...
                'simpleKenyaCheckout' => isSimpleKenyaCheckout(),
                'checkoutMpesaPhone' => session('checkout_mpesa_phone'),
            ]);
        }
